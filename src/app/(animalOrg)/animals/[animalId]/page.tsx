...
export default function Animal({ params }: { params: { animalId: string } }) {
  ...
  const [candidates, setcandidates] =
    useState<ICandidateSimple[]>(mockCandidates);
  ...
  const handleAdopt = async () => {
    try {
      const response = await put(
        `/api/v1/animals/adopt/${params.animalId}`,
        !animal.isAdopted
      );
      setAnimal(response);
    } catch (error) {
  ...
  const handleAdoptionEvent = async () => {
    try {
      const response = await put(
        `/api/v1/animals/adoption-event/${params.animalId}`,
        !animal.isAtEvent
      );
      setAnimal(response);
    } catch (error) {
  ...
  return (
    <div className="w-full flex flex-col items-center">
      <div className="flex flex-col items-center pt-20 sm:w-3/4 lg:w-2/3 md:w-1/2 xl:w-1/3">
        <div className="bg-secondary w-full flex flex-row items-center px-5 pt-4 pb-36">
          ...
          <div className="mx-3 flex-grow my-6">
            ...
            <p className="text-white text-xs font-light">
              {animal.type.toUpperCase()} | {animal.ageGroup.toUpperCase()}
            </p>
          ...
          <div
            className="flex flex-col items-center rounded-lg bg-white cursor-pointer p-[1px]"
          ...
          >
            ...
            <div className="flex mb-2">
              <Image
                ...
                height={15}
              />
              <label className="text-black text-sm cursor-pointer ml-1">
                Exportar
              </label>
            </div>
          </div>
        ...
        <div className="px-4 w-full -m-12">
          <div className="flex flex-col gap-2 -mt-20">
            ...
            >
              {animal.isAtEvent
                ? "Retornar da feira de adoção"
                : "Levar para feira de adoção"}
            </div>
          </div>
        </div>
          ...
          <div className="flex items-start justify-between bg-white shadow-md rounded-lg ">
            <div className="flex items-center justify-between">
              <div className="flex flex-col items-center justify-center m-5">
                <Image
                  src="/icons/cat-dog-vet.svg"
                  alt="Animais"
                  width={50}
                  height={50}
                />
                <p className="text-secondary font-extrabold text-xs mt-2">
                  SITUAÇÃO
                </p>
      ...
      <div className=" w-full lg:w-2/3 md:w-1/2 sm:w-3/4 xl:w-1/3 mb-10">
        <p className="text-secondary font-bold text-xl mx-5 pb-5">
          Candidatos à adoção
        </p>
