...
import { IAdoptionFormData } from "@/types";
...
export default function AdoptionForm({params}: {
	...
	const emptyInitialValues: IAdoptionFormData = {
		personalInformation: {
			identification: {
				name: "",
				cpf: "",
    ...
    const [initialValues, setInitialValues] = useState<IAdoptionFormData>(emptyInitialValues);

	useEffect(() => {
        get(`/api/v1/adoptions/${params.candidateId}`)
        .then((response) => {
